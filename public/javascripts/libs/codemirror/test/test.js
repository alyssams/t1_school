...
testCM("markTextSingleLine", function (cm) {
    forEach([{
        ...
        var r = cm.markText(Pos(0, 3), Pos(0, 6), {
            className: "foo"
        });
...
testCM("markTextAllowEmpty", function (cm) {
    var m1 = cm.markText(Pos(0, 1), Pos(0, 2), {
    ...
    is(!m2.find());
    var m3 = cm.markText(Pos(0, 1), Pos(0, 1), {
        clearWhenEmpty: false
    });
...
testCM("markTextStacked", function (cm) {
    var m1 = cm.markText(Pos(0, 0), Pos(0, 0), {
        clearWhenEmpty: false
    });
    var m2 = cm.markText(Pos(0, 0), Pos(0, 0), {
        clearWhenEmpty: false
    });
    cm.replaceRange("B", Pos(0, 1));
    is(m1.find() && m2.find());
...
testCM("bookmarkInsertLeft", function (cm) {
    var br = cm.setBookmark(Pos(0, 2), {
        insertLeft: false
    });
    var bl = cm.setBookmark(Pos(0, 2), {
        insertLeft: true
    });
...
testCM("bookmarkCursor", function (cm) {
    ...
    cm.setBookmark(Pos(1, 1), {
        widget: document.createTextNode("→")
    });
    cm.setBookmark(Pos(3, 0), {
        widget: document.createTextNode("→")
    });
...
testCM("multiBookmarkCursor", function (cm) {
    ...
    function add(insertLeft) {
        for (var i = 0; i < 3; ++i) {
            ...
            node.innerHTML = "X";
            ms.push(cm.setBookmark(Pos(0, 1), {
                widget: node,
...
testCM("randomCollapsedRanges", function (cm) {
    ...
    cm.operation(function () {
        for (var i = 0; i < 200; i++) {
            ...
            if (i % 4)
                try {
                    cm.markText(start, Pos(start.line + 2, 1), {
                        collapsed: true
                    });
                } catch (e) {
                    if (!/overlapping/.test(String(e))) throw e;
                } else
                    cm.markText(start, Pos(start.line, start.ch + 4), {
                        "className": "foo"
                    });
        }
    });
...
testCM("badNestedFold", function (cm) {
    addDoc(cm, 4, 4);
    cm.markText(Pos(0, 2), Pos(3, 2), {
        collapsed: true
    });
    var caught;
    try {
        cm.markText(Pos(0, 1), Pos(0, 3), {
            collapsed: true
        });
    } catch (e) {
        caught = e;
...
testCM("nestedFoldOnSide", function (cm) {
    ...
    var m3 = cm.markText(Pos(2, 0), Pos(2, 1), {
        collapsed: true
    });
    ...
    m1.clear();
    m4.clear();
    m1 = cm.markText(Pos(0, 1), Pos(2, 1), {
        collapsed: true
    });
...
testCM("editInFold", function (cm) {
    addDoc(cm, 4, 6);
    var m = cm.markText(Pos(1, 2), Pos(3, 2), {
        collapsed: true
    });
...
testCM("changedInlineWidget", function (cm) {
    ...
    var w = document.createElement("span");
    w.innerHTML = "x";
    var m = cm.markText(Pos(0, 4), Pos(0, 5), {
        replacedWith: w
    });
...
testCM("rtlMovement", function (cm) {
    ...
    forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج",
        ...
        "<img src=\"/בדיקה3.jpg\">", "يتم السحب في 05 فبراير 2014"
    ], function (line) {
...
testCM("jumpTheGap", function (cm) {
    ...
    cm.addLineWidget(1, node.cloneNode(true), {
        above: true
    });
    cm.setCursor(Pos(0, 2));
    cm.execCommand("goLineDown");
...
testCM("selectionHomeEnd", function (cm) {
    cm.markText(Pos(1, 0), Pos(1, 1), {
        atomic: true,
        inclusiveLeft: true
    });
    cm.markText(Pos(1, 3), Pos(1, 4), {
        atomic: true,
        inclusiveRight: true
    });
